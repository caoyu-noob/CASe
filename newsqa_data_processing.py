...
''' Basically copyed from https://github.com/Maluuba/newsqa/blob/master/maluuba/newsqa/data_processing.py
'''
...
class NewsQaDataset(object):
    def __init__(self, cnn_stories_path=None, dataset_path=None, log_level=logging.INFO,
        ...
        with io.open(os.path.join(dirname, 'newsqa/stories_requiring_extra_newline.csv'),
                     'r', encoding='utf-8') as f:
            stories_requiring_extra_newline = set(f.read().split('\n'))

        with io.open(os.path.join(dirname, 'newsqa/stories_requiring_two_extra_newlines.csv'),
                     'r', encoding='utf-8') as f:
            stories_requiring_two_extra_newlines = set(f.read().split('\n'))

        with io.open(os.path.join(dirname, 'newsqa/stories_to_decode_specially.csv'),
                     'r', encoding='utf-8') as f:
                ...
                for member in t.getmembers():
                    ...
                    if story_id in remaining_story_ids:
                        ...
                        if story_id in stories_to_decode_specially:
                            lines = map(lambda s: u"".join(six.unichr(c) for c in s.strip()),
    ...
    def to_dict(self):
        ...
        train_story_ids = set(
            pd.read_csv(os.path.join(dir_name, 'newsqa/train_story_ids.csv'))['story_id'].values)
        dev_story_ids = set(
            pd.read_csv(os.path.join(dir_name, 'newsqa/dev_story_ids.csv'))['story_id'].values)
        test_story_ids = set(
            pd.read_csv(os.path.join(dir_name, 'newsqa/test_story_ids.csv'))['story_id'].values)
